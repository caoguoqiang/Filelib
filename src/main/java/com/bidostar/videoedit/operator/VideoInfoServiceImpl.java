...
/**
 * Created by smallc on 2017/4/5 21:01.
 */

public class VideoInfoServiceImpl {
    private final String TAG = "VideoOperator";
    private MediaMetadataRetriever mMetadataRetriever;

    public VideoInfoServiceImpl(String path) {
        setFilePath(path);
    }
    ...
    public boolean setFilePath(String path){
        ...
        if (!file.exists()) {
            Log.e(TAG,"path file   not exists !" + path);
            return false;
        }
        try{
    ...
    public Bitmap extractFrame() {
        return mMetadataRetriever!= null ?mMetadataRetriever.getFrameAtTime(): null;
    }
    /**
     * 获取视频某一帧,不一定是关键帧，不耗时
     *
    ...
    public String getDate(){
        return mMetadataRetriever.extractMetadata(MediaMetadataRetriever.METADATA_KEY_DATE);
    }
